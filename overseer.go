...
type Config struct {
	// Required will prevent overseer from fallback to running the program in the main process on failure.
	Required bool

	// Program's main function
	Program func(state State)

	// Address for program's zero-downtime socket listening (set this or Addresses)
	Address string

	// Addresses ofr program's zero-downtime socket listening (set this or Address)
	Addresses []string

	// RestartSignal will manually trigger a graceful restart. Defaults to SIGUSR2.
	RestartSignal os.Signal

	// TerminateTimeout controls how long overseer should wait for the program to terminate itself.
	// After this timeout, overseer will issue a SIGKILL.
	TerminateTimeout time.Duration

	// MinFetchInterval defines the smallest duration between Fetch()s. This helps to prevent unwieldy fetch.
	// Interfaces from hogging too many resources. Defaults to 1 second.
	MinFetchInterval time.Duration

	// PreUpgrade runs after a binary has been retrieved, user defined checks can be run here and returning
	// an error will cancel the upgrade.
	...
	// NoRestart disables all restarts, this option essentially converts the RestartSignal into a "ShutdownSignal".
	NoRestart bool
...
// Run executes overseer, if an error is encountered, overseer falls-back to running
// the program directly (unless Config.Required is set).
func Run(c Config) {
	err := runErr(&c)
	if err != nil {
		if c.Required {
...
// SanityCheck manually runs the check to ensure this binary is compatible with overseer.
// This tries to ensure that a restart is never performed against a bad binary,
// as it would require manual intervention to rectify. This is automatically done
...
// Restart programmatically triggers a graceful restart. If Config.NoRestart is enabled,
// then this will essentially be a graceful shutdown.
func Restart() {
